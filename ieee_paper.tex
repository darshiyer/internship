...
\begin{itemize}
    ...
    \item \textbf{No study has compared English-pivot, Hindi-pivot, and
    direct (pivot-free) translation for the Marathi--Kannada direction.}
    English is linguistically distant from both languages; Hindi
    (Indo-Aryan, Devanagari script, $\sim$60\% lexical overlap with
    Marathi) is a far more natural intermediate language.
...
This study addresses all four gaps. The pivot-strategy comparison
(English vs.\ Hindi vs.\ direct) constitutes the primary novel
...
We implement and compare \textbf{three distinct translation strategies}
for the Marathi$\leftrightarrow$Kannada direction, evaluated on 500
held-out sentence pairs on Google Colab (T4 GPU).

\subsubsection{System A — English Pivot (baseline)}
\begin{equation}
\text{MR} \xrightarrow{\text{IndicTrans2}} \text{EN}
\xrightarrow{\text{NLLB-200}} \text{KN}
\end{equation}
Two separate models with English as intermediate. Error propagates
across both hops; English grammar distortion is injected into the
intermediate representation.

\subsubsection{System B — Hindi Pivot (proposed)}
\begin{equation}
\text{MR} \xrightarrow{\text{IndicTrans2 Indic-Indic}} \text{HI}
\xrightarrow{\text{IndicTrans2 Indic-Indic}} \text{KN}
\end{equation}
Hindi is Indo-Aryan like Marathi, shares Devanagari script, and has
$\sim$60\% lexical overlap with Marathi~\cite{masica1991indoaryan}.
The MR$\rightarrow$HI hop is high-quality and nearly lossless;
HI$\rightarrow$KN is better resourced than direct MR$\rightarrow$KN.
Both hops use \texttt{ai4bharat/indictrans2-indic-indic-1B},
which translates directly between any two of 22 Indian languages
\textbf{without touching English}.

\subsubsection{System C — Direct Translation (proposed, no pivot)}
\begin{equation}
\text{MR} \xrightarrow{\text{IndicTrans2 Indic-Indic}} \text{KN}
\end{equation}
A single-hop direct translation using
\texttt{ai4bharat/indictrans2-indic-indic-1B}.
No pivot language involved. This is the theoretically cleanest
approach — no error propagation, no structural distortion from an
intermediate language. Requires sufficient MR--KN training signal
in the base model; our 128,460 MR--KN aligned pairs from the
quadruplet corpus are used to fine-tune this direction.

\noindent\textbf{Models used:}
\begin{itemize}
    \item \texttt{ai4bharat/indictrans2-indic-en-1B}: Indic$\rightarrow$English
    (System A, hop 1)~\cite{gala2023indictrans2}
    \item \texttt{facebook/nllb-200-distilled-600M}: English$\rightarrow$Indic
    (System A, hop 2)~\cite{nllb2022}
    \item \texttt{ai4bharat/indictrans2-indic-indic-1B}: Direct Indic$\rightarrow$Indic
    (Systems B and C)~\cite{gala2023indictrans2}
\end{itemize}
...
\subsection{Translation Pipeline Comparison}
Table~\ref{tab:translation} compares all three translation strategies
on the 500-sentence held-out test set.
System~C (direct, no pivot) is our primary novel contribution.
Systems~A and~B serve as baselines to quantify
the benefit of removing English from the translation chain.

\begin{table}[h]
\centering
\caption{Translation quality comparison across three pipeline strategies
(500-sentence held-out test set, MR$\rightarrow$KN direction).
Bold = best result per metric.}
\label{tab:translation}
\renewcommand{\arraystretch}{1.2}
\begin{tabular}{lcccc}
\toprule
\textbf{System} & \textbf{BLEU} & \textbf{chrF} & \textbf{BERT-F1} & \textbf{RT-cos} \\
\midrule
A: MR$\to$EN$\to$KN (English pivot)  & 2.77 & 32.50 & 0.787 & 0.898 \\
B: MR$\to$HI$\to$KN (Hindi pivot)   & \textit{TBD} & \textit{TBD} & \textit{TBD} & \textit{TBD} \\
C: MR$\to$KN (Direct, no pivot)      & \textit{TBD} & \textit{TBD} & \textit{TBD} & \textit{TBD} \\
\midrule
A: KN$\to$EN$\to$MR (English pivot)  & 7.44 & 32.41 & 0.786 & --- \\
B: KN$\to$HI$\to$MR (Hindi pivot)   & \textit{TBD} & \textit{TBD} & \textit{TBD} & --- \\
C: KN$\to$MR (Direct, no pivot)      & \textit{TBD} & \textit{TBD} & \textit{TBD} & --- \\
\bottomrule
\multicolumn{5}{l}{\small RT-cos = round-trip cosine similarity (semantic preservation).} \\
\multicolumn{5}{l}{\small \textit{TBD} = pending execution on Colab T4 GPU.}
\end{tabular}
\end{table}
